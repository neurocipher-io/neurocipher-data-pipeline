...
class TestRLSScanChain:
    ...
    def test_tenant_isolation_with_correct_account(self, db_connection, setup_test_data):
        ...
        with db_connection.cursor() as cur:
            ...
            assert count_rows(cur, "evidence") == 1, "Expected 1 evidence for acct_scan_1"
            assert count_rows(cur, "remediation") == 1, "Expected 1 remediation for acct_scan_1"
            assert count_rows(cur, "ticket") == 1, "Expected 1 ticket for acct_scan_1"
            assert count_rows(cur, "integration") == 1, "Expected 1 integration for acct_scan_1"
            assert count_rows(cur, "notification") == 1, "Expected 1 notification for acct_scan_1"
    ...
    def test_tenant_isolation_with_wrong_account(self, db_connection, setup_test_data):
        ...
        with db_connection.cursor() as cur:
            ...
            assert count_rows(cur, "evidence") == 0, "Expected 0 evidence for acct_other"
            assert count_rows(cur, "remediation") == 0, "Expected 0 remediations for acct_other"
            assert count_rows(cur, "ticket") == 0, "Expected 0 tickets for acct_other"
            assert count_rows(cur, "integration") == 0, "Expected 0 integrations for acct_other"
            assert count_rows(cur, "notification") == 0, "Expected 0 notifications for acct_other"
    ...
    def test_tenant_isolation_with_null_account(self, db_connection, setup_test_data):
        ...
        with db_connection.cursor() as cur:
            ...
            assert count_rows(cur, "policy") == 0, "Expected 0 policies with null context"
            assert count_rows(cur, "finding") == 0, "Expected 0 findings with null context"
            assert count_rows(cur, "evidence") == 0, "Expected 0 evidence with null context"
            assert count_rows(cur, "remediation") == 0, "Expected 0 remediations with null context"
            assert count_rows(cur, "ticket") == 0, "Expected 0 tickets with null context"
            assert count_rows(cur, "integration") == 0, "Expected 0 integrations with null context"
            assert count_rows(cur, "notification") == 0, "Expected 0 notifications with null context"
    ...
    def test_scan_finding_chain_integrity(self, db_connection, setup_test_data):
        ...
        with db_connection.cursor() as cur:
            ...
            assert remediation[1] == "find_001", "Remediation should be linked to find_001"
