...
## 2. Context
Multiple services (ingest, normalize, embed, query, and the compliance scanners described in `docs/integrations/`) require coordinated workflows, rollouts, and backfills. Prior drafts showed ambiguity in which layer owned orchestration logic and how boundaries mapped to repositories, IAM, and APIs. This ADR fixes the boundary map and standardizes orchestration responsibilities.

## 3. Decision
- **The external orchestrator documented in `docs/integrations/README.md` is the orchestration tier** for Neurocipher Core and partner modules.
...
| **Compliance Module (see integrations catalog)** | CSPM scanning, rules engine, reports | AWS | Security Eng |
| **External Orchestrator (see integrations catalog)** | Workflow coordination, rollouts, backfills, task routing | AWS | Platform Eng |
