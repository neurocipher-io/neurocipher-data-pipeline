...
Foundation for AWS native ingestion, normalization, embeddings, and hybrid retrieval.
...
  N --> E[SQS embed]
  ...
  N --> SE[Security Engine contracts]
...
  API->>API: Fuse results via RRF
  ...
  opt Security remediation loop
    Norm->>SE: emit event.security.finding.v1
    SE->>API: POST /v1/security/actions (cmd.security.*)
    API->>SE: emit event.security.action_status.v1
  end
```
...
10. Security Engine interface

- Events emitted: `event.security.finding.v1` and `event.security.anomaly.v1` from normalization/serving stages when policies fire (schemas live in `schemas/events/`).
- Commands accepted: `/v1/security/actions` ingests `cmd.security.quarantine.v1`, `cmd.security.ticket.create.v1`, and `cmd.security.notify.v1` with `action_id` idempotency.
- Status callbacks: `/v1/security/actions/{id}` and `event.security.action_status.v1` broadcast lifecycle transitions with unique `status_id` values.
- SLOs: p95 decision latency ≤ 90 s, queue age ≤ 120 s, ≥ 99.5 % successful callback rate, at-least-once delivery with dedupe on `action_id`.
- Integration details live in `docs/integrations/Security-Engine-Integration.md`; runtime design stays under `docs/security-engine/`.
