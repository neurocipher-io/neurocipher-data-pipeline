...
**Owner**: Platform  
**Scope**: Neurocipher Pipeline on AWS. Partner-specific ingress flows are documented in `docs/integrations/README.md`.  
**Status**: Final v1.0  
...
- Partner subdomains (see integrations catalog)
...
- `/v1/security/actions` (POST) accepts `cmd.security.quarantine.v1`, `cmd.security.ticket.create.v1`, or `cmd.security.notify.v1` payloads defined under `schemas/events/`.
- `/v1/security/actions/{id}` (GET) returns the latest `event.security.action_status.v1` for a given `action_id`.
- Standard headers apply (`Tenant-Id`, `Correlation-Id`, `Idempotency-Key`), and both endpoints emit `429` when back-pressure is required.
- Runtime remediation logic stays inside the Security Engine module; the gateway enforces auth, rate limits, and schema validation only.
